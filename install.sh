...
# Function to set up the addCreds repository and alias
setup_add_creds() {
    local add_creds_dir="$HOME/.addCreds"
    local add_creds_repo="git@github.com:dvdknaap/add-creds-rofi.git"
    local add_creds_alias="alias addCreds='bash ~/.addCreds/scripts/add_creds'"
    ...
    echo "Checking if the alias for addCreds is set..."
    local shell_rc
    ...
    if ! grep -q "$add_creds_alias" "$shell_rc"; then
        echo "Alias not found. Adding to $shell_rc..."
        echo "$add_creds_alias" >> "$shell_rc"
        echo "Sourcing $shell_rc to apply changes..."
        source "$shell_rc"
    else
        echo "Alias for addCreds is already set."
    fi
}
...
main() {
    ...
    # Set up addCreds repository and alias
    setup_add_creds
